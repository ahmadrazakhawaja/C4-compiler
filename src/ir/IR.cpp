...
private:
    ...
    TypeDesc typeFromTypeNode(const Node::Ptr& typeNode) {
        ...
        ast::TypeSpec ts = detail::buildTypeSpecFromNode(typeNode);
        TypeDesc base = typeFromTypeSpec(ts);
        Node::Ptr adNode = detail::findFirstNodeOfType(typeNode, abstractdeclarator);
        if (adNode) {
            ast::AbstractDeclarator ad = detail::buildAbstractDeclaratorFromNode(adNode);
